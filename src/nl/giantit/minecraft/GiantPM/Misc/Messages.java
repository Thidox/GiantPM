...
public class Messages {
	...
	private double yamlVersion = 1.0;
	...
	private void loadMain() {
		...
		if(list == null) {
			GiantPM.log.log(Level.WARNING, "[" + plugin.getName() + "] There are no main messages specified in the msgTemplates.yml file. This might cause errors!");
			return;
		}
	...
	private void loadAdmin() {
		...
		if(list == null) {
			GiantPM.log.log(Level.WARNING, "[" + plugin.getName() + "] There are no admin messages specified in the msgTemplates.yml file. This might cause errors!");
			return;
		}
	...
	private void loadErrors() {
		...
		if(list == null) {
			GiantPM.log.log(Level.WARNING, "[" + plugin.getName() + "] There are no error messages specified in the msgTemplates.yml file. This might cause errors!");
			return;
		}
	...
	public Messages(GiantPM plugin) {
		this.plugin = plugin;
		
		File configFile = new File(plugin.getDir(), "msgTemplate.yml");
		if(!configFile.exists()) {
			GiantPM.log.log(Level.INFO, "[" + plugin.getName() + "] Extracting new msgTemplate.yml file...");
		...
		if(v < this.yamlVersion) {
			GiantPM.log.log(Level.INFO, "[" + plugin.getName() + "] Your msgTemplate.yml has ran out of date. Updating now!");
	...
	public String getMsg(msgType type, String template, HashMap<String, String> data) {
		...
		GiantPM.log.log(Level.SEVERE, "[" + plugin.getName() + "] Template for " + template + " does not exist!");
